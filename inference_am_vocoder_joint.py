...
def main(args, config):
    ...
    for file in files:
        ...
        for i, (speaker, prompt, text, content) in enumerate(
            tqdm(zip(speakers, prompts, texts, contents))
        ):
            ...
            sequence = (
                torch.from_numpy(np.array(text_int)).to(device).long().unsqueeze(0)
            )
            sequence_len = torch.from_numpy(np.array([len(text_int)])).to(device)
            style_embedding = torch.from_numpy(style_embedding).to(device).unsqueeze(0)
            content_embedding = (
                torch.from_numpy(content_embedding).to(device).unsqueeze(0)
            )
            ...
            with torch.no_grad():
                ...
                if not os.path.exists(root_path + "/test_audio/audio/" + f"{file}/"):
                    os.makedirs(
                        root_path + "/test_audio/audio/" + f"{file}/", exist_ok=True
                    )
                sf.write(
                    file=root_path + "/test_audio/audio/" + f"{file}/{i+1}.wav",
                    data=audio,
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        required=False,
        default="",
        help="inference specific checkpoint, e.g --checkpoint checkpoint_230000",
    )
    p.add_argument(
        "-t",
        "--test_file",
        type=str,
        required=True,
        help="the absolute path of test file that is going to inference",
    )

    args = p.parse_args()
    ##################################################
    sys.path.append(
        os.path.dirname(os.path.abspath("__file__")) + "/" + args.config_folder
    )
